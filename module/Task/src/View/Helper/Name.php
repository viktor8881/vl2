...
class Name extends AbstractHelper
{
    ...
    public function __invoke(Task $model)
    {
        ...
        if ($model->isPercent()) {
            $xhtml .= '<div class="col-sm-12">' . sprintf(
                    _('%1$s на %2$s за %3$s'), $modeName,
                    $this->view->formatPercent($model->getPercent(), true),
                    $this->view->pluralDays($model->getPeriod(), true)
                ) . '</div>';
        } elseif ($model->isOvertime()) {
            $xhtml .= '<div class="col-sm-12">' . sprintf(
                    _('%1$s в течении %2$s'), $modeName,
                    $this->view->pluralDaysGenitive($model->getPeriod(), true)
                ) . '</div>';
        }
